...
let agentContextState = new Map(); // conversation_id -> AgentContextMetadata
let selectedContextConversation = null; // conversation_id currently displayed
...
const contextConversationLabel = document.getElementById('contextConversationLabel');
...
function handleEvent(event) {
    ...
    // Handle agent_context events separately
    if (event.event_type === 'agent_context') {
        handleAgentContextEvent(event);
        return; // Don't add to activity log
    }

    // Handle memory change events
    if (event.event_type === 'observation_extracted' ||
        event.event_type === 'summary_generated' ||
        event.event_type === 'observation_created') {
        handleMemoryChangeEvent(event);
        return;
    }
...
/**
 * Handle memory change events (observations, summaries)
 */
function handleMemoryChangeEvent(event) {
    // Add to activity log
    appendToActivityLog(event);

    // If this profile has selected conversation, show refresh notification
    if (selectedContextConversation) {
        const conv = conversations.get(selectedContextConversation);
        if (conv && conv.profile_id === event.profile_id) {
            showContextRefreshNotification();
        }
    }
}

/**
 * Show visual notification that context was updated
 */
function showContextRefreshNotification() {
    const header = document.querySelector('.card-header.bg-primary');
    if (header && header.textContent.includes('Agent Context')) {
        header.classList.add('context-updated');
        setTimeout(() => header.classList.remove('context-updated'), 2000);
    }
}
...
function createConversationCard(convId) {
    ...
    // Make card clickable to select context
    card.addEventListener('click', () => {
        selectConversationContext(convId);
    });
...
function updateCardContent(convId) {
    ...
    // Build event list - filter to only show user messages and AI responses
    const filteredEvents = conversation.events.filter(event =>
        event.event_type === 'user_message' || event.event_type === 'ai_response'
    );

    const eventListHtml = filteredEvents.map(event => {
        let messagePreview = '';
        if (event.message) {
...
/**
 * Handle agent context update events
 */
function handleAgentContextEvent(event) {
    const convId = event.conversation_id;
    agentContextState.set(convId, event.metadata);

    // Auto-select first conversation if none selected
    if (!selectedContextConversation && conversations.size > 0) {
        selectConversationContext(convId);
    }

    // Re-render if this is the selected conversation
    if (selectedContextConversation === convId) {
        renderAgentContext(convId);
    }
}

/**
 * Select a conversation to display context for
 */
function selectConversationContext(convId) {
    selectedContextConversation = convId;

    // Update visual selection
    document.querySelectorAll('.conversation-card').forEach(card => {
        card.classList.remove('selected-for-context');
    });
    const selectedCard = document.getElementById(`card-${convId}`);
    if (selectedCard) {
        selectedCard.classList.add('selected-for-context');
    }

    // Update label
    const shortId = convId.length > 15 ? '...' + convId.slice(-10) : convId;
    contextConversationLabel.textContent = `Conversation ${shortId}`;

    renderAgentContext(convId);
}

/**
 * Render agent context for a conversation
 */
function renderAgentContext(convId) {
    const context = agentContextState.get(convId);

    if (!context) {
        agentContextPanel.innerHTML = '<div class="text-center py-5 text-muted">No context available</div>';
        return;
    }
    ...
    if (context.profile) {
        html += `
            <div class="mb-3">
                ...
                <div class="small">
                    <div><strong>ID:</strong> <span class="font-monospace">${escapeHtml(context.profile.profile_id)}</span></div>
                    ${renderIdentifiers(context.profile.identifiers)}
                    ${renderTraits(context.profile.traits)}
                </div>
            </div>
        `;
    }

    // Observations Section - Show count even if empty
    html += `
        <div class="mb-3">
            <h6 class="border-bottom pb-2">Observations (${context.observations?.length || 0})</h6>
            <div class="small">
                ${context.observations && context.observations.length > 0
                    ? context.observations.map(obs => `
                        <div class="card mb-2">
                            <div class="card-body p-2">
                                <div class="text-muted mb-1">${formatTimestamp(obs.created_at)}</div>
                                <div>${escapeHtml(obs.content)}</div>
                            </div>
                        </div>
                    `).join('')
                    : '<div class="text-muted">No observations yet</div>'
                }
            </div>
        </div>
    `;

    // Summaries Section - Show count even if empty
    html += `
        <div class="mb-3">
            <h6 class="border-bottom pb-2">Summaries (${context.summaries?.length || 0})</h6>
            <div class="small">
                ${context.summaries && context.summaries.length > 0
                    ? context.summaries.map(sum => `
                        <div class="card mb-2">
                            <div class="card-body p-2">
                                <div class="text-muted mb-1">${formatTimestamp(sum.created_at)}</div>
                                <div>${escapeHtml(sum.content)}</div>
                            </div>
                        </div>
                    `).join('')
                    : '<div class="text-muted">No summaries yet</div>'
                }
            </div>
        </div>
    `;
    ...
    if (context.communications && context.communications.length > 0) {
        html += `
            <div class="mb-3">
                <h6 class="border-bottom pb-2">Communications (${context.communications.length})</h6>
                <div class="small">
                    ${context.communications.map(comm => `
                        <div class="card mb-2">
                            <div class="card-body p-2">
                                <div class="d-flex justify-content-between mb-1">
                                    <strong>${escapeHtml(comm.author_name)}</strong>
                                    <span class="text-muted">${formatTimestamp(comm.created_at)}</span>
                                </div>
                                <div>${escapeHtml(comm.content)}</div>
                            </div>
                        </div>
                    `).join('')}
                </div>
    ...
    agentContextPanel.innerHTML = html || '<div class="text-center py-5 text-muted">No data available</div>';
}

/**
 * Render profile identifiers
 */
...
/**
 * Render profile traits (nested objects)
 */
function renderTraits(traits) {
    ...
    function renderTraitValue(value, depth = 0) {
...
function getEventLabel(eventType) {
    const labels = {
        ...
        'observation_extracted': 'Observation detected',
...
function getEventBadgeClass(eventType) {
    const classes = {
        ...
        'call_setup': 'bg-info',
        'observation_extracted': 'bg-info',
        'summary_generated': 'bg-primary',
        'observation_created': 'bg-success'
    };
